...
"""
    ...
    def parse_response(response, chunk, book, **kwargs):
        ...
        try:
            ...
            # 这个模块初衷：处理一个章节的时候，把 上个章节/ 下个章节的 附加信息 提供一些。增加它的判断力
            # 为了尽快调通，我们先临时屏蔽，等以后再研究优化。目前的方法估计实现有问题
            # 处理 next chunk starting point
            if False and response['next_chunk_start']:
            ...
            for unprocessed_plot in response['plots']:
                ...
                # 寻找原始句子，主要是怕LLM抄录的源句子缺少 字符，导致 字符串匹配失败
...
def extract(book, chunk_size=8192):  # 从book中提取信息
    ...
    print("本书 chunk切分完毕，一共{a}个chunk-章节".format(a=len(chunk_generator)))
...
def assemble(book):
    ...
    2. Enhances[增强] conversation scenarios[场景] and character motivations using LLM
    3. Normalizes and standardizes character names # 人名规范化
    4. Generates character profiles and datasets # 生成角色的 背景描述和 数据
    5. Saves the final assembled data  # 完成最终的组装数据
...
"""
            ...
            # 用增强后的对话，来更改原本的对话，起始就是增加了 [想法][动作][语言]
    ...
    # Filter out plots with invalid character data  # 过滤无效的对话角色(比如无中生有了一些角色)
    plots = [
        p for p in plots 
